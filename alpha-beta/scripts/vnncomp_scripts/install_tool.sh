...
/root/miniconda/bin/conda env create --name alpha-beta-crown -f ${TOOL_DIR}/complete_verifier/environment_pyt111.yaml
/root/miniconda/bin/conda env create --name alpha-beta-crown-2022 -f ${TOOL_DIR}/complete_verifier/environment_2022.yaml
python3 -m pip install -U --no-deps git+https://github.com/dlshriver/DNNV.git@4d4b124bd739b4ddc8c68fed1af3f85b90386155#egg=dnnv
...
#if [ "$( python3 -c 'import torch; print(torch.__version__)')" != '1.11.0' ]; then
...
then
    ...
    tar xvfz gurobi9.5.1_linux64.tar.gz -C /opt
fi
...
# Setup Gurobi
grbprobe_output=$(/opt/gurobi951/linux64/bin/grbprobe)
