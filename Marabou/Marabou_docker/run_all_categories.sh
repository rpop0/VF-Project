...
#export PYTHONPATH=PYTHONPATH:/Marabou/maraboupy/
...
then
    ...
    echo "errored_vnncomp" > $RESULT_CSV_FILE
    exit 0
fi
...
then
    ...
    echo "errored_tool_$TOOL_FOLDER" > $RESULT_CSV_FILE
    exit 0
fi
...
do
    ...
    then
	    ...
	    echo "errored_instances" > $RESULT_CSV_FILE
	    exit 0
    fi
	...
	if [[ $CATEGORY != "cifar2020" && $CATEGORY != "test" ]]; then
	    ...
	    echo "errored_category" > $RESULT_CSV_FILE
	    exit 0
	else
